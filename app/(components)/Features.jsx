...
export const Features = () => {
  ...
  const features = [
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 sm:h-10 sm:w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"> {/* Adjusted stroke width */}
  ...
  // Slightly thinner stroke for icons can look better in dark mode
  // You can adjust strokeWidth="1.5" in the SVGs above if you like

  return (
    // --- Dark Mode Changes Start Here ---
    <section className="py-16 md:py-24 bg-gray-900 text-gray-300" id="features"> {/* Dark background, lighter base text */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="w-20 h-1 bg-[#007BFF] mx-auto mt-6"></div> {/* Accent line remains */}
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          >
            {/* Darker list container background, adjusted border */}
            <div className="bg-gray-800 rounded-lg overflow-hidden shadow-sm border border-gray-700">
              {features.map((feature, index) => (
                <motion.div
                  ...
                  // Dark theme styles for list items
                  className={`p-4 sm:p-5 cursor-pointer transition-colors duration-200 flex items-center border-l-4 ${
                    activeFeature === index
                      ? "bg-gray-700 border-[#007BFF]" // Active: Slightly lighter dark bg, accent border
                      : "border-transparent hover:bg-gray-700/50" // Inactive: Transparent border, dark hover
                  } ${index !== features.length -1 ? 'border-b border-gray-600' : ''}`} // Darker separator border
                >
                  {/* Icon color remains accent, adjust opacity */}
                  <div className={`text-[#007BFF] mr-4 flex-shrink-0 transition-opacity duration-200 ${activeFeature === index ? "opacity-100" : "opacity-70"}`}>
                    {feature.icon}
                  </div>
                  <div>
                     {/* Text color changes based on active state */}
                    <h3 className={`font-semibold text-base sm:text-lg transition-colors duration-200 ${
                      activeFeature === index ? "text-[#007BFF]" : "text-gray-200" // Active: Accent color, Inactive: Light gray
                    }`}>
                      {feature.title}
                    </h3>
                  </div>
          ...
          <motion.div
            ...
            key={activeFeature} // Key ensures re-animation on change
            className="lg:col-span-8"
          >
            {/* Darker card background, adjusted border */}
            <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-700 flex flex-col">
              {/* Header gradient can remain, contrasts well */}
              ...
              <div className="p-5 sm:p-6 flex-grow">
                <div className="mb-5 sm:mb-6">
                  ...
                  <p className="text-gray-400 text-sm sm:text-base">{features[activeFeature].description}</p>
                </div>

                {/* Benefit Box: Dark theme adjustments */}
                <div className="bg-gray-700 p-4 sm:p-5 rounded-lg border-l-4 border-blue-500 shadow-sm">
                   <div className="flex items-start">
                       {/* Use a lighter accent color for the icon */}
                      ...
                      <div>
                          {/* Lighter text colors for benefit box */}
                          <h4 className="text-base sm:text-lg font-semibold text-blue-300 mb-1">Your Benefit</h4>
                          <p className="text-blue-400 text-sm sm:text-base font-medium">{features[activeFeature].benefit}</p>
                      </div>
                   </div>
                </div>
              </div>
